...
const TRANSLATIONS = {
  home: {
    ...
    text: [
      ...
      [".work__box:nth-of-type(5) h3", "BearFit Multi-Sensor Fitness Demo", "BearFit 多传感器健身演示"],
      [".work__box:nth-of-type(5) p", "A gym-ready fitness sensing demo where normal clothing is enough: one phone, one watch, and one pair of earbuds drive rep tracking and live feedback without extra wearables or setup.", "一个面向健身房场景的健身感知演示系统，正常穿着即可使用：一部手机、一块手表和一副耳机就能完成计次和实时反馈，无需额外穿戴设备或复杂准备。"],
      [".work__box:nth-of-type(5) .link__text", "Project details", "项目详情"],
      ...
      [".timeline-column:nth-child(1) .timeline__item:nth-child(3) .timeline__meta", "2019 - 2021", "2019 - 2021"],
      [".timeline-column:nth-child(1) .timeline__item:nth-child(3) h4", "PLA Army Special Operations Brigade", "中国人民解放军陆军特种作战某旅"],
      [".timeline-column:nth-child(1) .timeline__item:nth-child(3) p:nth-child(3)", "Received the Four Haves Excellent Soldier Medal and the Dedication to National Defense Bronze Medal.", "获四有优秀士兵勋章、献身国防铜质勋章。"],
  ...
  bearfit: {
    title: { en: "BearFit Multi-Sensor Fitness Demo | Chunpo Wu", zh: "BearFit 多传感器健身演示 | 吴春坡" },
    description: { en: "BearFit multi-sensor fitness demo by Chunpo Wu.", zh: "吴春坡的 BearFit 多传感器健身演示项目。" },
    text: [
      [".nav__item:nth-child(1) .nav__link", "Projects", "项目"],
      [".nav__item:nth-child(2) .nav__link", "Experience", "经历"],
      [".nav__item:nth-child(3) .nav__link", "Blog", "博客"],
      [".nav__item:nth-child(4) .nav__link", "About", "关于"],
      [".nav__item:nth-child(5) .nav__link", "Contact", "联系"],
      [".page-hero .heading-primary", "BearFit Multi-Sensor Fitness Demo", "BearFit 多传感器健身演示"],
      [".page-hero p", "A gym-ready workout tracking prototype built around what people already carry: one phone, one watch, and one pair of earbuds, with no special outfit or external rig required.", "一个围绕用户现有设备构建的健身房训练追踪原型：一部手机、一块手表和一副耳机即可完成使用，不需要特殊穿着或外部支架。"],
      [".project-detail__content h2:nth-of-type(1)", "Overview", "概述"],
      [".project-detail__content > p", "BearFit is designed around a low-friction gym workflow: you walk in dressed normally, bring your iPhone, wear an Apple Watch, put on your earbuds, and the system can start tracking movement immediately without straps, markers, or dedicated hardware.", "BearFit 围绕一种低摩擦的健身房使用流程设计：你只需要正常穿着，带上 iPhone、戴上 Apple Watch、再戴上一副耳机，系统就可以立即开始追踪动作，不需要绑带、标记点或专用硬件。"],
      [".project-detail__content > p:nth-of-type(2)", "Technically, the project fuses motion streams from the phone, Apple Watch, and earbuds, then adds local logging, MQTT streaming for low-latency Linux delivery, a FastAPI-based inference backend, and offline movement-analysis scripts for repetition progress, completion features, and model training experiments.", "在技术实现上，项目融合了手机、Apple Watch 和耳机的动作数据流，并在此基础上加入本地日志记录、面向 Linux 的低延迟 MQTT 流传输、基于 FastAPI 的推理后端，以及用于计次进度、完成度特征和模型训练实验的离线动作分析脚本。"],
      [".project-detail__content h2:nth-of-type(2)", "Video", "视频"],
      [".project-detail__content h2:nth-of-type(3)", "Key Work", "关键工作"],
      [".project-detail__content li:nth-child(1)", "Built an iPhone + Apple Watch data pipeline for collecting synchronized IMU streams during workouts.", "构建了一个 iPhone + Apple Watch 的数据管线，用于在训练过程中采集同步 IMU 数据流。"],
      [".project-detail__content li:nth-child(2)", "Integrated earbud motion sensing so a normal phone + watch + earbuds setup is enough for live exercise tracking.", "整合了耳机动作感知能力，使一套普通的手机 + 手表 + 耳机组合就足以完成实时训练追踪。"],
      [".project-detail__content li:nth-child(3)", "Added MQTT-based live streaming so multiple Linux receivers can subscribe to the same sensor session.", "加入了基于 MQTT 的实时流传输，使多台 Linux 接收端可以订阅同一条传感器会话。"],
      [".project-detail__content li:nth-child(4)", "Implemented a FastAPI backend for ingest, logging, and exercise-classifier inference experiments.", "实现了一个 FastAPI 后端，用于数据接收、日志写入和动作分类推理实验。"],
      [".project-detail__content li:nth-child(5)", "Created offline movement-analysis tooling for progress baselines, completion features, and visualization.", "构建了用于进度基线、完成度特征和可视化的离线动作分析工具链。"],
      [".project-detail__aside h3", "Project Info", "项目信息"],
      [".project-detail__aside dt:nth-of-type(1)", "Platform", "平台"],
      [".project-detail__aside dd:nth-of-type(1)", "iPhone, Apple Watch, AirPods, Linux backend", "iPhone、Apple Watch、AirPods、Linux 后端"],
      [".project-detail__aside dt:nth-of-type(2)", "Frontend", "前端"],
      [".project-detail__aside dd:nth-of-type(2)", "SwiftUI, WatchConnectivity, Core Motion", "SwiftUI、WatchConnectivity、Core Motion"],
      [".project-detail__aside dt:nth-of-type(3)", "Backend", "后端"],
      [".project-detail__aside dd:nth-of-type(3)", "Python, FastAPI, MQTT", "Python、FastAPI、MQTT"],
      [".project-detail__aside dt:nth-of-type(4)", "Focus", "重点"],
      [".project-detail__aside dd:nth-of-type(4)", "Multi-sensor fusion, rep tracking, live feedback, local-first exercise analysis", "多传感器融合、计次追踪、实时反馈、本地优先的训练分析"],
    ],
    attrs: [
      ["title", "text", "BearFit Multi-Sensor Fitness Demo | Chunpo Wu", "BearFit 多传感器健身演示 | 吴春坡"],
      ["meta[name='description']", "content", "BearFit multi-sensor fitness demo by Chunpo Wu.", "吴春坡的 BearFit 多传感器健身演示项目。"],
      [".project-back-link", "aria-label", "Back to projects", "返回项目列表"],
    ],
  },
};
