...
const TRANSLATIONS = {
  home: {
    ...
    text: [
      ...
      [".work__box:nth-of-type(5) h3", "BearFit Multi-Device IMU Fitness Sensing", "BearFit 多设备 IMU 健身感知系统"],
      [".work__box:nth-of-type(5) p", "A multi-device IMU fitness sensing system for body-movement awareness, combining phone, watch, earbuds, and ring data for workout tracking and live feedback in normal gym use.", "一个面向健身房场景的多设备 IMU 健身感知系统，融合手机、手表、耳机与戒指数据，用于身体动作感知、训练追踪与实时反馈。"],
      ...
      [".timeline__item--military .timeline__meta", "2019 - 2021", "2019 - 2021"],
      [".timeline__item--military h4", "PLA Army Special Operations Brigade", "中国人民解放军陆军特种作战某旅"],
      [".timeline__item--military p:last-child", "Received the Four Haves Excellent Soldier Medal and the Dedication to National Defense Bronze Medal.", "获四有优秀士兵勋章、献身国防铜质勋章。"],
      ...
      [".adventurex-award .timeline__title-link", "AdventureX Hackathon, Track Award", "AdventureX 黑客松赛道奖项"],
      [".timeline-column:nth-child(2) .timeline__item:nth-child(6) .timeline__meta", "Publication", "论文"],
      [".publication-insulator .timeline__title-link", "Lightweight Insulator Defect Detection Algorithm Based on UAV Perspective", "基于无人机视角的轻量化绝缘子缺陷检测算法"],
  ...
  bearfit: {
    title: { en: "BearFit Multi-Device IMU Fitness Sensing | Chunpo Wu", zh: "BearFit 多设备 IMU 健身感知系统 | 吴春坡" },
    description: { en: "BearFit multi-device IMU fitness sensing project by Chunpo Wu.", zh: "吴春坡的 BearFit 多设备 IMU 健身感知项目。" },
    text: [
      [".nav__item:nth-child(1) .nav__link", "Projects", "项目"],
      [".nav__item:nth-child(2) .nav__link", "Experience", "经历"],
      [".nav__item:nth-child(3) .nav__link", "Blog", "博客"],
      ...
      [".project-detail__content h2:nth-of-type(1)", "Overview", "概述"],
      [".project-detail__content > p", "BearFit is designed around a low-friction gym workflow: you walk in dressed normally, bring your iPhone, wear an Apple Watch, put on your earbuds, and add a lightweight IMU ring, then the system can start tracking whole-body movement immediately without cameras, straps, or dedicated gym hardware.", "BearFit 围绕一种低摩擦的健身房使用流程设计：你只需要正常穿着，带上 iPhone、戴上 Apple Watch、佩戴耳机并加上一枚轻量 IMU 戒指，系统就可以立即开始追踪全身动作，不需要摄像头、绑带或专用健身硬件。"],
      [".project-detail__content > p:nth-of-type(2)", "Technically, the project fuses motion streams from the phone, Apple Watch, earbuds, and left/right ring IMUs, then adds local logging, MQTT streaming for low-latency Linux delivery, a FastAPI-based inference backend, and offline movement-analysis scripts for repetition progress, completion features, pose fusion, and model training experiments.", "在技术实现上，项目融合了手机、Apple Watch、耳机以及左右戒指 IMU 的动作数据流，并在此基础上加入本地日志记录、面向 Linux 的低延迟 MQTT 流传输、基于 FastAPI 的推理后端，以及用于计次进度、完成度特征、姿态融合和模型训练实验的离线动作分析脚本。"],
      [".project-detail__content h2:nth-of-type(2)", "Video", "视频"],
      [".project-detail__content h2:nth-of-type(3)", "Key Work", "关键工作"],
      [".project-detail__content li:nth-child(1)", "Built a multi-device IMU pipeline for collecting synchronized motion streams from phone, watch, earbuds, and rings during workouts.", "构建了一个多设备 IMU 数据管线，用于在训练过程中同步采集手机、手表、耳机与戒指的动作数据流。"],
      [".project-detail__content li:nth-child(2)", "Extended the sensing setup with ring_left and ring_right inputs to capture finer hand and arm motion during exercise.", "在感知链路中加入 ring_left 和 ring_right 输入，以捕捉训练过程中更细粒度的手部与手臂动作。"],
      [".project-detail__content li:nth-child(3)", "Added MQTT-based live streaming so multiple Linux receivers can subscribe to the same sensor session with low latency.", "加入了基于 MQTT 的实时流传输，使多台 Linux 接收端可以低延迟订阅同一条传感器会话。"],
      [".project-detail__content li:nth-child(4)", "Implemented a FastAPI backend for ingest, logging, exercise inference experiments, and body-movement analysis workflows.", "实现了一个基于 FastAPI 的后端，用于数据接入、日志记录、训练推理实验和身体动作分析流程。"],
      [".project-detail__content li:nth-child(5)", "Created offline tooling for progress baselines, completion features, pose fusion, and visualization across multi-device IMU sessions.", "构建了离线工具链，用于多设备 IMU 会话下的计次进度基线、完成度特征、姿态融合与可视化。"],
      [".project-detail__aside h3", "Project Info", "项目信息"],
      [".project-detail__aside dt:nth-of-type(1)", "Platform", "平台"],
      [".project-detail__aside dd:nth-of-type(1)", "iPhone, Apple Watch, AirPods, IMU rings, Linux backend", "iPhone、Apple Watch、AirPods、IMU 戒指、Linux 后端"],
      ...
      [".project-detail__aside dt:nth-of-type(4)", "Focus", "重点"],
      [".project-detail__aside dd:nth-of-type(4)", "Multi-device IMU fusion, body-movement sensing, rep tracking, live feedback", "多设备 IMU 融合、身体动作感知、计次追踪、实时反馈"],
    ],
    attrs: [
      ["title", "text", "BearFit Multi-Device IMU Fitness Sensing | Chunpo Wu", "BearFit 多设备 IMU 健身感知系统 | 吴春坡"],
      ["meta[name='description']", "content", "BearFit multi-device IMU fitness sensing project by Chunpo Wu.", "吴春坡的 BearFit 多设备 IMU 健身感知项目。"],
      [".project-back-link", "aria-label", "Back to projects", "返回项目列表"],
    ],
  },
};
