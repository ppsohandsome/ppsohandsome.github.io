...
const TRANSLATIONS = {
  home: {
    ...
    text: [
      ...
      [".work__box:nth-of-type(5) p", "A cross-device fitness sensing demo that combines iPhone, Apple Watch, AirPods, on-device pose tracking, and a Python backend for motion capture, rep counting, and live feedback.", "一个跨设备健身感知演示项目，结合了 iPhone、Apple Watch、AirPods、端侧姿态跟踪和 Python 后端，用于动作采集、计次和实时反馈。"],
  ...
  bearfit: {
    ...
    text: [
      ...
      [".page-hero p", "A multi-device workout tracking prototype that fuses iPhone, Apple Watch, AirPods, camera-based pose estimation, and a Linux backend into one fitness sensing workflow.", "一个多设备训练追踪原型，将 iPhone、Apple Watch、AirPods、基于相机的姿态估计和 Linux 后端整合进同一条健身感知流程。"],
      [".project-detail__content h2:nth-of-type(1)", "Overview", "概述"],
      [".project-detail__content > p", "BearFit is built as a sensor-first fitness demo rather than a single-mode app. The iPhone app can ingest motion data from the phone itself, AirPods IMU streams, and Apple Watch wrist motion, while also supporting a camera mode for live pose estimation and form analysis.", "BearFit 不是单一模式的健身应用，而是一个以传感器为核心的健身演示系统。iPhone 端可以接收手机自身、AirPods IMU 和 Apple Watch 手腕动作的数据，同时支持用于实时姿态估计和动作分析的相机模式。"],
      [".project-detail__content > p:nth-of-type(2)", "Around that capture layer, the project adds local logging, MQTT streaming for low-latency delivery to Linux machines, a FastAPI-based inference backend, and offline movement-analysis scripts for repetition progress, completion features, and model training experiments.", "围绕这层采集能力，项目还加入了本地日志记录、面向 Linux 机器的低延迟 MQTT 流传输、基于 FastAPI 的推理后端，以及用于计次进度、完成度特征和模型训练实验的离线动作分析脚本。"],
      [".project-detail__content h2:nth-of-type(2)", "Video", "视频"],
      [".project-detail__content h2:nth-of-type(3)", "Key Work", "关键工作"],
      ...
      [".project-detail__content li:nth-child(2)", "Integrated AirPods motion sensing and camera-based pose tracking into two different exercise modes.", "将 AirPods 动作感知和基于相机的姿态跟踪整合进两种不同的训练模式。"],
